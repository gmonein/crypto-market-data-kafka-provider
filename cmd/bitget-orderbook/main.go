...
var (
	...
	depthFlag    = flag.Int("depth", 15, "Bitget orderbook depth (5, 15, or 0 for full)")
)

type bitgetOrderBookEnvelope struct {
...
func main() {
	...
	if topic == "" {
		if symbolNorm == "BTCUSDT" || symbolNorm == "BTCUSD" {
			topic = "bitget_orderbook"
		} else {
			topic = "bitget_" + symbolLower + "_orderbook"
		}
	}
	...
	log.Printf("Starting Bitget Orderbook Follower. Brokers: %v, Topic: %s, Symbol: %s, Channel: %s", brokers, topic, symbolNorm, channel)
	...
	fetchSnapshot := func(ctx context.Context) ([][]string, [][]string, int64, error) {
		url := "https://api.bitget.com/api/v2/spot/market/orderbook?symbol=" + subSymbol + "&limit=" + strconv.Itoa(restDepth)
		...
		if payload.Data.Ts != "" {
			if parsed, err := strconv.ParseInt(payload.Data.Ts, 10, 64); err == nil {
				ts = parsed
			}
		}
		return payload.Data.Bids, payload.Data.Asks, ts, nil
	}
	...
	for {
		...
		go func() {
			...
			for {
				...
				var env bitgetOrderBookEnvelope
